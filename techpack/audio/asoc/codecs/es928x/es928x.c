...
static DECLARE_TLV_DB_SCALE(adc_tlv, -12750, 50, 0);
static DECLARE_TLV_DB_SCALE(dac_tlv, -255, 0, 0);
...
static const struct snd_kcontrol_new es928x_controls[] = {
	...
	SOC_SINGLE_RANGE_TLV("DAC 1 Volume", ES928X_REG_DAC_VOL_1,
			0, ES928X_DAC_VOLUME1_MASK, ES928X_DAC_VOLUME1_SHIFT, 0, dac_tlv),
	SOC_SINGLE_RANGE_TLV("DAC 2 Volume", ES928X_REG_DAC_VOL_2,
			0, ES928X_DAC_VOLUME2_MASK, ES928X_DAC_VOLUME2_SHIFT, 0, dac_tlv),
...
static void es928x_shut_down(struct es928x_priv *es928x)
{
	...
	//msleep_interruptible(500); //SS This should not be required
...
static void es928x_fw_get_version_work_fn(struct work_struct *work)
{
	...
	dev_info(es928x->dev, "MODULE_VERSION 1.8.5 \n");
	for(i=0;i<7;i++) {
...
{
	struct es928x_jdet_priv *es928x_jdet = 
		container_of(work, struct es928x_jdet_priv, zdet_work);
	struct snd_soc_component *component = es928x_jdet->component;
	struct es928x_priv *es928x = snd_soc_component_get_drvdata(component);
	u8 zdet_val[3];
...
void es928x_jdet_jack_det(struct snd_soc_component *component, struct snd_soc_jack *jack)
{
	...
	while(((boot_flag & ES928X_BOOT_FINISHED_FLAG) != ES928X_BOOT_FINISHED_FLAG) || ret)
	{
		...
		if(wait++ > 7)
		{
				dev_err(es928x->dev, "%s cannot get boot flag. Firwmare is not loaded.\n", __func__);
				break;
		}
		ret = regmap_read(es928x->regmap, ES928X_REG_MCU_INTERRUPT_FLAGS, &boot_flag);
	}
...
int es928x_jdet_init(struct snd_soc_component *component)
{
	...
	INIT_WORK(&es928x_jdet->zdet_work, es928x_zdet_work_fn);
...
#define ES928X_CDEV_NAME     "es928x_node"
